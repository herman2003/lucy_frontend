...
- [ ] **`validate-answer` après chaque saisie** ; si `valid: true`, **`turnSummary` + confirmation utilisateur** avant question suivante.
- [ ] Si `valid: false` : **`rephrasedQuestion` remplace** le libellé (pas « Peux-tu préciser ») ; max **10** tentatives / question.
- [ ] **« Ce n’est pas ça »** après `turnSummary` → retour saisie sans `confirm-turn`.
- [ ] **`confirm-turn`** après « C’est bon » ; Nest écrit Firestore ; miroir local.
- [ ] 10 échecs validate → **`fallbackSummary`** + validation / rendu réduit si refus.
- [ ] `analyze` + fallback profil après 10 échecs ; **`finalize`** pour clôturer.
- [ ] **7 chats** (1 step = 1 fil) ; typing Lucy pendant appels IA ; steps futurs **verrouillés**.
- [ ] Design messagerie (bulles, avatar, `colorScheme`) ; swipe entre steps **terminés** + courant.
- [ ] Retour arrière : modifier un tour n’efface pas les autres.
- [ ] Réponses ≤ **2000 caractères** ; boutons désactivés pendant appels (widget partagé).
- [ ] Guard router : `isConfigured` / `onboardingStatus` lus depuis Firestore.
- [ ] Confirmation obligatoire avant Firestore.
- [ ] `onboardingTranscript` + `learnerProfile` + `isConfigured: true` après validation.
- [ ] Pas de skip ; garde `/home` si `false`.
- [ ] l10n fr/en/de ; Clean Architecture ; pas de message API brut.

#### Backend

- [ ] Catalogue `questionId` + `locale` côté serveur ; client n’envoie pas `questionText`.
- [ ] `validate-answer` : vague → `valid: false` + `rephrasedQuestion` ; clair → `valid: true` + **`turnSummary` obligatoire**.
- [ ] `isConfigured === true` → **403** sur les deux routes.
- [ ] `analyze` : lit Firestore, 7 entrées → 200 + `learnerProfile` §4.4.1.
- [ ] Transcript Firestore &lt; 7 → `ONBOARDING_TRANSCRIPT_INCOMPLETE`.
- [ ] &gt; 2000 caractères → `ANSWER_TOO_LONG` ; catalogue `questions/` ; `confirm-turn` + `finalize`.
- [ ] Nest **Admin** : seul writer onboarding ; Flutter read + local mirror.
- [ ] JSON IA invalide → 502 ; profil incomplet → 422.
- [ ] Deux prompts distincts dans `src/prompts/`.
- [ ] Tests : `OnboardingService` + `LlmPort` mocké (validate + analyze).
